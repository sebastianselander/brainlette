...
data TopDef' a = FnDef a (Type' a) Ident [Arg' a] (Blk' a)
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type Arg = Arg' BNFC'Position
data Arg' a = Argument a (Type' a) Ident
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type Blk = Blk' BNFC'Position
...
data Type' a
    = Int a | Doub a | Bool a | Void a | Fun a (Type' a) [Type' a]
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
instance HasPosition Blk where
  hasPosition = \case
    Block p _ -> p
